        ...
        if st.button("🚀 Analizi Başlat", type="primary", use_container_width=True):
            with st.spinner("🤖 Sanal Planner analiz ediyor..."):
                ...
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    st.session_state['trading'] = trading_df
                    st.session_state['urun'] = urun_df
                    st.session_state['rapor'] = rapor
                    st.session_state['sevk_df'] = sevk_df
                    st.session_state['indirim_df'] = indirim_df
...
with tab2:
    ...
    if 'analiz_yapildi' not in st.session_state:
        ...
        # Özet metrikler
        sevk_df = st.session_state['sevk_df']
        indirim_df = st.session_state['indirim_df']
...
with tab3:
    ...
    if 'sevk_df' not in st.session_state:
...
with tab4:
    ...
    if 'indirim_df' not in st.session_state:
        st.info("⬆️ Önce 'Veri Yükle' sekmesinden analizi başlat.")
    else:
        indirim_df = st.session_state['indirim_df']
